...
void loop()
{
  ...
  if (!digitalRead(PIN_NEW_MEASUREMENT))
  {
    createNewMeassurement();
    while (!digitalRead(PIN_NEW_MEASUREMENT))
      ;
  }
...
void meassurement()
{
  ...
  sprintf(txtMess, "%lld, %lld, %.02f, %s, %s, %d",
          ...
          meassurement_values.powers[0] ? "true" : "false",
          meassurement_values.powers[1] ? "true" : "false",
          meassurement_cause);
